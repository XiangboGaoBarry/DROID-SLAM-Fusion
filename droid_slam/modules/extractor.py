...
if __name__ != '__main__':
    ...
    from fusion import SelfAttFuser, DeformAttFuser
...
class DualEncoder(nn.Module):
    def __init__(self, output_dim=128, norm_fn='batch', dropout=0.0, multidim=False, args=None, ori_shape=(640,320)):
        ...
        if self.args.fusion_method == 'self_att':
            ...
            self.fuser1 = DeformAttFuser(DIM, H=ori_shape[0]//2, W=ori_shape[1]//2)
            self.fuser2 = DeformAttFuser(2*DIM, H=ori_shape[0]//2, W=ori_shape[1]//2)
            self.fuser3 = DeformAttFuser(4*DIM, H=ori_shape[0]//2, W=ori_shape[1]//2)
    ...
    def forwardFusion(self, x):
        b, n, c1, h1, w1 = x.shape
        assert n == 2, 'DualEncoder requires two inputs'
        x1 = x[:, 0]
        x2 = x[:, 1]
        ...
        x1 = self.encoder1.conv2(x1)
        x2 = self.encoder2.conv2(x2)
